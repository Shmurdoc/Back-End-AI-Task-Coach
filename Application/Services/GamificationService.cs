...
namespace Application.Services
{
    public class GamificationService : IGamificationService
    {
        ...
        public Task<UserBadge?> AwardBadgeAsync(Guid userId, string badgeType, string reason)
        {
            _logger.LogInformation("Awarding badge {BadgeType} to user {UserId} for: {Reason}", badgeType, userId, reason);
            
            var result = badgeType.ToLower() switch
            {
                ...
                "streak-master" => new UserBadge(Guid.NewGuid(), "Streak Master", "Consistent daily completion", "/badges/streak-master.png", DateTime.UtcNow, reason, BadgeRarity.Epic),
                "hall-of-fame" => new UserBadge(Guid.NewGuid(), "Hall of Fame", "Legendary achievements", "/badges/hall-of-fame.png", DateTime.UtcNow, reason, BadgeRarity.Legendary),
                _ => null
            };
        ...
        public Task<List<LeaderboardEntry>> GetLeaderboardAsync(LeaderboardType type, int limit = 10)
        {
            ...
            var result = new List<LeaderboardEntry>
            {
                ...
                new LeaderboardEntry(Guid.NewGuid(), "TaskNinja", 2100, 2),
                new LeaderboardEntry(Guid.NewGuid(), "FocusedPro", 1850, 3),
                new LeaderboardEntry(Guid.NewGuid(), "GoalCrusher", 1600, 4),
                new LeaderboardEntry(Guid.NewGuid(), "HabitBuilder", 1400, 5)
            }.Take(limit).ToList();
            return Task.FromResult(result);
        }
